...
tf.flags.DEFINE_string('exp_name', 'exp', 'experiment name')
...
tf.flags.DEFINE_string('logdir', '/tmp/logs', 'log Directory.')
...
tf.flags.DEFINE_boolean('cutout', True, 'wether to use cutout')
tf.flags.DEFINE_boolean('autoaug', False, 'wether to use autoaug')
tf.flags.DEFINE_integer('use_cpu', 1, '1 if use CPU, else GPU.')
tf.flags.DEFINE_string('index_file', None, '1 if use CPU, else GPU.')
tf.flags.DEFINE_integer('train_batch', 128, '1 if use CPU, else GPU.')
tf.flags.DEFINE_integer('test_batch', 25, '1 if use CPU, else GPU.')
tf.flags.DEFINE_string('train_dir', None, '1 if use CPU, else GPU.')
tf.flags.DEFINE_string('action_files', None, '1 if use CPU, else GPU.')
tf.flags.DEFINE_integer('train_epoch', 200, '1 if use CPU, else GPU.')
tf.flags.DEFINE_integer('n_action', 9, '1 if use CPU, else GPU.')
tf.flags.DEFINE_integer('n_step', 10, '1 if use CPU, else GPU.')
...
# fh = logging.FileHandler('logs/' + FLAGS.exp_name + '.log')
fh = logging.FileHandler(FLAGS.logdir + '/' + FLAGS.exp_name + '.log')
log.addHandler(fh)
...
def build_model(inputs, num_classes, is_training, hparams):
  ...
  with nested(*scopes):
    ...
    elif 'wrn' in hparams.model_name:
      logits = build_wrn_model(
          inputs, num_classes, hparams.wrn_size, hparams.depth)
...
class CifarModel(object):
  ...
  def _setup_misc(self, mode):
    ...
    if mode == 'eval':
      self.batch_size = self.hparams.test_batch
    # self.batch_size_tf = tf.placeholder(tf.int32, shape=[])
...
class CifarModelTrainer(object):
  ...
  def __init__(self, hparams):
    ...
    self.model_dir = os.path.join(FLAGS.checkpoint_dir, FLAGS.exp_name)
...
def main(_):
  ...
  if (not (FLAGS.index_file is None)) and FLAGS.index_file != '':
    hparams.add_hparam('index_file', FLAGS.index_file)
  hparams.add_hparam('cutout', FLAGS.cutout)
  hparams.add_hparam('autoaug', FLAGS.autoaug)
  hparams.add_hparam('train_batch', FLAGS.train_batch)
  hparams.add_hparam('test_batch', FLAGS.test_batch)
  hparams.add_hparam('train_dir', FLAGS.train_dir)
  hparams.add_hparam('action_files', FLAGS.action_files)
  hparams.add_hparam('n_action', FLAGS.n_action)
  hparams.add_hparam('n_step', FLAGS.n_step)
  if hparams.autoaug:
    tf.logging.info('Using Auto-augment policy')
  if hparams.cutout:
    tf.logging.info('Using cutout policy')
  if FLAGS.model_name == 'wrn':
    hparams.add_hparam('depth', 28)
    hparams.add_hparam('num_epochs', 200)
    hparams.add_hparam('model_name', 'wrn')
    #print('############ Number of epochs %d' % (FLAGS.train_epoch))
    hparams.add_hparam('wrn_size', 160)
    hparams.add_hparam('lr', 0.1)
    hparams.add_hparam('weight_decay_rate', 5e-4)
  elif FLAGS.model_name == 'wrn-28-2':
    hparams.add_hparam('depth', 28)
    hparams.add_hparam('num_epochs', 200)
    hparams.add_hparam('model_name', 'wrn')
    ...
    hparams.add_hparam('lr', 0.1)
    hparams.add_hparam('weight_decay_rate', 5e-4)
  elif FLAGS.model_name == 'wrn-40-2':
    hparams.add_hparam('depth', 40)
    hparams.add_hparam('num_epochs', 200)
    hparams.add_hparam('model_name', 'wrn')
    hparams.add_hparam('wrn_size', 32)
    hparams.add_hparam('lr', 0.1)
